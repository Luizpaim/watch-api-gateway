...
describe('AuthService unit tests', () => {
  ...
  beforeEach(async () => {
    ...
    const entity = new UserEntity(
      UserDataBuilder({ email: 'a@a.com', password: 'skljlsadlkjaslkdj' }),
    )

    entityMapper = UserOutputMapper.toOutput(entity)
  })
  ...
  it('should return a jwt', async () => {
    const result = await sut.generateJwt(entityMapper)

    const { password, ...safeUser } = entityMapper
